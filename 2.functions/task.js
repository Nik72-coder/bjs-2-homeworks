function getArrayParams(...arr) {
  if (arr.length === 0) {
    return 0;
  }
  let min = arr[0];
  let max = arr[0];
  let average = 0;
  let sum = 0;
    for (let i = 0; i < arr.length; i++) {
      if (min > arr[i]) {
        min = arr[i];
      };
      if (max < arr[i]) {
        max = arr[i];
      }
      sum += arr[i];
    }
    average = Number((sum / arr.length).toFixed(2))
    return {min: min, max: max, avg: average}
}


function summElementsWorker(...arr) {
  if (arr.length === 0) {
    return 0;
  }
  let sum = 0;
    for (let i = 0; i < arr.length; i++) {
    sum += arr[i];
    }
    return sum;
}



function differenceMaxMinWorker(...arr) {
  if (arr.length === 0) {
    return 0;
  }
    let min = arr[0];
    let max = arr[0];
    for (let i = 0; i < arr.length; i++) {
      if (min > arr[i]) {
        min = arr[i];
      };
      if (max < arr[i]) {
        max = arr[i];
      }
    } 
    return max - min; 
}



function differenceEvenOddWorker(...arr) {
  if (arr.length === 0) {
    return 0;
  }
    let sumEvenElement = 0;
    let sumOddElement = 0;
    for (let i = 0; i < arr.length; i++) {
      if (arr[i] % 2 === 0) {
        sumEvenElement += arr[i];
      } else {
        sumOddElement += arr[i];
      }
    }
    return sumEvenElement - sumOddElement;
}



function averageEvenElementsWorker(...arr) {
  if (arr.length === 0) {
    return 0;
  }
    let sumEvenElement = 0;
    let countEvenElement = 0;
    for (let i = 0; i < arr.length; i += 1) {
      if (arr[i] % 2 === 0) {
          sumEvenElement += arr[i];
          countEvenElement += 1;
        }
      }
      return sumEvenElement / countEvenElement;
}


function makeWork(arrOfArr, func) {
  let maxWorkerResult = arrOfArr[0][0];
  console.log('maxWorkerResult = ', maxWorkerResult)
    for(let i = 0; i < arrOfArr.length; i++) {
      for(let j = 0; j < arrOfArr[i].length; j++) {
        let currentNumber = func(arrOfArr[i][j]);
        if(maxWorkerResult < currentNumber) {
          maxWorkerResult = currentNumber;
        } 
      }
    }
  return maxWorkerResult;
};

const arr = [[10, 10, 11, 20, 10], [67, 10, 2, 39, 88], [72, 75, 51, 87, 43], [30, 41, 55, 96, 62]];

// console.log(makeWork(arr, summElementsWorker(...arr)))
console.log(summElementsWorker(...arr[0]))
console.log(summElementsWorker(...[10, 10, 11, 20, 10]));
console.log(summElementsWorker(10, 10, 11, 20, 10));
console.log(summElementsWorker(67, 10, 2, 39, 88))